...
class ReqeustGetBookInfo(BaseModel):
...
class BookInfoResponse(BaseModel):
    ...
    subtitle: Optional[str] = Field(title="subtitle", description="부제목", example="for beginner")
    ...
    image_url: Optional[str] = Field(title="image_url", description="책 이미지 링크", example="https://example.com/img")
    version: Optional[str] = Field(title="version", description="판본", example="10e")
    major: Optional[bool] = Field(title="major", description="전공책 여부", example=True)
